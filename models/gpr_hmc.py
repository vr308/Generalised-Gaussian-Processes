...
class GPR_HMC(gpytorch.models.ExactGP):
   ...
   def __init__(self, train_x, train_y, likelihood):
        ...
        super(GPR_HMC, self).__init__(train_x, train_y, likelihood)
        self.train_x = train_x
        self.train_y = train_y
        self.data_dim = self.train_x.shape[1]                                                                                          
        self.mean_module = ZeroMean()
        self.covar_module = ScaleKernel(RBFKernel(ard_num_dims=self.data_dim))
   ...
   def sample_optimal_variational_hyper_dist(self, n_samples, input_dim, tune):
       ...
       with pm.Model() as model_pymc3:
            ...
            y_ = gp.marginal_likelihood("y", X=self.train_x.numpy(), y=self.train_y.numpy(), noise=sig_n)
...
def mixture_posterior_predictive(model, test_x, trace_hyper):
      ...
      for i in range(len(trace_hyper)):
         ...
         model.covar_module.outputscale = hyper_sample['sig_f']**2
         model.covar_module.base_kernel.lengthscale = hyper_sample['ls']
...
if __name__ == '__main__':
    ...
    model = GPR_HMC(X_train,Y_train, likelihood)
